...
def forecast_IS(year, year_data, prev_yr_data):
  ...
  # Forecasted Data
  revGrowth = round(((year_data['Revenue'] / prev_yr_data['Revenue']) - 1) * 100, 1)
  grossMargin = round((1 - (year_data['Cost of Goods Sold'] / year_data['Revenue'])) * 100, 1)
  operatingMargin = round((year_data['Operating Income (EBIT)'] / year_data['Revenue']) * 100, 1)
  taxRate = round((year_data['Taxes'] / year_data['Operating Income (EBIT)']) * 100, 1)
  # revGrowthRate = (year_data['Revenue'] / prev_yr_data['Revenue']) - 1 * 100

  forecast_data = {
    'Year': year,
    'Forecasted Revenue Growth': f'{revGrowth}%',
    'Forecasted Gross Margin' : f'{grossMargin}%',
    'Forecasted Operating Margin': f'{operatingMargin}%',
    'Forecasted Historical Tax Rate': f'{taxRate}%',
    'Forecasted Historical Interest Rate': None
  }
  
  # Forecasted Growth Rates
  revGrowthRate = round(((year_data['Revenue'] / prev_yr_data['Revenue']) - 1) * 100, 1)
  grossProfitRate = round(((year_data['Gross Profit'] / prev_yr_data['Gross Profit']) - 1) * 100, 1)
  ebitRate = round(((year_data['Operating Income (EBIT)'] / prev_yr_data['Operating Income (EBIT)']) - 1) * 100, 1)
  netIncomeRate = round(((year_data['Net Income'] / prev_yr_data['Net Income']) - 1) * 100, 1)


  yr_over_yr_growth_rates_data = {
    'Year': year,
    'Revenue': f'{revGrowthRate}%',
    'Gross Profit' : f'{grossProfitRate}%',
    'EBIT': f'{ebitRate}%',
    'Net Income': f'{netIncomeRate}%'
  }

  # Forecast Margin Ratios
  cogsRatio = round((year_data['Cost of Goods Sold'] / year_data['Revenue']) * 100, 1)
  grossMarginRatio = round((year_data['Gross Profit'] / year_data['Revenue']) * 100, 1)
  rdRatio = round((year_data['Research & Development'] / year_data['Revenue']) * 100, 1)
  salesRatio = round((year_data['Sales & Marketing'] / year_data['Revenue']) * 100, 1)
  genAdminRatio = round((year_data['General and Admin Expenses'] / year_data['Revenue']) * 100, 1)
  depAppRatio = round((year_data['Depreciation & Amortization'] / year_data['Revenue']) * 100, 1)
  taxRatio = round((year_data['Taxes'] / year_data['Operating Income (EBIT)']) * 100, 1)

  margin_ratios = {
    'Year': year,
    'COGs': f'{cogsRatio}%',
    'Gross Profit' : f'{grossMarginRatio}%',
    'R&D': f'{rdRatio}%',
    'S&M': f'{salesRatio}%',
    'G&A': f'{genAdminRatio}%',
    'D&A': f'{depAppRatio}%',
